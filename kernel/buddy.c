...
static void *freespace_start;
static void *freespace_end;
...
// my add
//switch bit at positioin index in array
...
bd_malloc(uint64 nbytes)
{
  ...
  bit_switch(bd_sizes[k].alloc, blk_index(k, p));
  ...
  for(; k > fk; k--) {
    ...
    bit_switch(bd_sizes[k-1].alloc, blk_index(k-1, p));
...
bd_free(void *p) {
  ...
  for (k = size(p); k < MAXSIZE; k++) {
    ...
    bit_switch(bd_sizes[k].alloc,bi);  // free p at size k
...
bd_mark(void *start, void *stop)
{
  ...
  for (int k = 0; k < nsizes; k++) {
    ...
    for(; bi < bj; bi++) {
      ...
      bit_switch(bd_sizes[k].alloc, bi);
    }
  }
}
...
bd_init(void *base, void *end) {
  ...
  freespace_start=p;
  freespace_end=end;
  printf("the start: %p,the end: %p and the length is %d\n",freespace_start,freespace_end,freespace_end-freespace_start);
  // initialize free lists for each size k
  int free = bd_initfree(p, bd_end);// real free space for me is p to bd_end
